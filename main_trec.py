            ...
            if rel == label.index('Live_In-->'):
                if torch.argmax(out_e[0]) == 7:
                    loss += weights[0,0] * ((out_s[0, 3] -\
                        F.sigmoid(self.linear_con((rel_vec[rel] + \
...
emb = cPickle.load(open("data/TREC/embedding300_glove", "rb"))
idxs_train = cPickle.load(open("data/TREC/idx.train", "rb"))
idxs_test = cPickle.load(open("data/TREC/idx.test", "rb"))
labels_train, labels_type_train = cPickle.load(open("data/TREC/labels_chunk.train", "rb"))
labels_test, labels_type_test = cPickle.load(open("data/TREC/labels_chunk.test", "rb"))
labels_rel_train = cPickle.load(open("data/TREC/labels_2rel.train", "rb"))
labels_rel_test = cPickle.load(open("data/TREC/labels_2rel.test", "rb"))
pos_train, pos_test = cPickle.load(open("data/TREC/pos.tag", "rb"))
pos_dic = ['pad']
...
f_out = open("result/TREC/transformer-logic.txt", "w")
